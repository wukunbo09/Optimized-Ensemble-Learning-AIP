...
def process_file(file_path):
    ...
    important_features = np.argsort(feature_importance)[::-1][:int(len(feature_importance) * 0.2)]  # 选择前50%的特征
    X_train_selected = X_train_res[:, important_features]
    X_test_selected = X_test[:, important_features]

    model = Sequential()
    model.add(Dense(256, input_shape=(X_train_selected.shape[1],), activation='relu'))
    ...
    model.fit(X_train_selected, y_train_res, epochs=50, batch_size=32, verbose=0, validation_split=0.2)

    y_pred_proba = model.predict(X_test_selected).flatten()
